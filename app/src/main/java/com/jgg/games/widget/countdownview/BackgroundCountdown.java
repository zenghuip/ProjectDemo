...
class BackgroundCountdown extends BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeBgDivisionLineSize = ta.getDimension(R.styleable.CountdownView_timeBgDivisionLineSize, CountUtils.dp2px(context, DEFAULT_TIME_BG_DIVISION_LINE_SIZE));
        mTimeBgSize = ta.getDimension(R.styleable.CountdownView_timeBgSize, 0);
        mDefSetTimeBgSize = mTimeBgSize;
        mTimeBgBorderSize = ta.getDimension(R.styleable.CountdownView_timeBgBorderSize, CountUtils.dp2px(context, DEFAULT_TIME_BG_BORDER_SIZE));
    ...
    protected void initTimeTextBaseInfo() {
        ...
        if (mDefSetTimeBgSize == 0 || mTimeBgSize < mTimeTextWidth) {
            mTimeBgSize = mTimeTextWidth + (CountUtils.dp2px(mContext, 2) * 4);
        }
    }
    ...
    private void initHasBackgroundTextBaseY(RectF rectF) {
        ...
        mTimeBgDivisionLineYPos = rectF.centerY() + (mTimeBgDivisionLineSize == CountUtils.dp2px(mContext, DEFAULT_TIME_BG_DIVISION_LINE_SIZE) ? mTimeBgDivisionLineSize : mTimeBgDivisionLineSize / 2);
    }
    ...
    public int getAllContentWidth() {
        ...
        if (isShowDay) {
            if (isDayLargeNinetyNine) {
                ...
                mDayTimeBgWidth = rect.width() + (CountUtils.dp2px(mContext, 2) * 4);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            ...
            // draw day text
            canvas.drawText(CountUtils.formatNum(mDay), mDayBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            // draw hour text
            canvas.drawText(CountUtils.formatNum(mHour), mHourBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowMinute) {
            ...
            // draw minute text
            canvas.drawText(CountUtils.formatNum(mMinute), mMinuteBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowSecond) {
            ...
            // draw second text
            canvas.drawText(CountUtils.formatNum(mSecond), mSecondBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(CountUtils.formatMillisecond(mMillisecond), mMillisecondBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
    ...
    public void setTimeBgSize(float size) {
        mTimeBgSize = CountUtils.dp2px(mContext, size);
    }
    ...
    public void setTimeBgRadius(float radius) {
        mTimeBgRadius = CountUtils.dp2px(mContext, radius);
    }
    ...
    public void setTimeBgDivisionLineSize(float size) {
        mTimeBgDivisionLineSize = CountUtils.dp2px(mContext, size);
        if (null != mTimeBgDivisionLinePaint) {
            mTimeBgDivisionLinePaint.setStrokeWidth(mTimeBgDivisionLineSize);
        }
    }
    ...
    public void setTimeBgBorderSize(float size) {
        mTimeBgBorderSize = CountUtils.dp2px(mContext, size);
    ...
    public void setTimeBgBorderRadius(float size) {
        mTimeBgBorderRadius = CountUtils.dp2px(mContext, size);
    }
