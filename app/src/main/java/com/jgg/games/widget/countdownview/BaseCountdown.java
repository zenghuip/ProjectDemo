...
class BaseCountdown {
    ...
    protected float mSuffixDayLeftMargin, mSuffixDayRightMargin,mSuffixTopMargin,suffixMargin;
    ...
    private float textBoundWidth;
    private boolean timeLineBottom; // 是否显示在正下方
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, CountUtils.sp2px(mContext, 12));
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, CountUtils.sp2px(mContext, 12));
        ...
        suffixMargin = ta.getDimension(R.styleable.CountdownView_suffixMargin, -1);
        mSuffixTopMargin = ta.getDimension(R.styleable.CountdownView_suffixTopMargin, -1);
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = CountUtils.dp2px(mContext, DEFAULT_SUFFIX_LR_MARGIN);
    ...
    private float getSuffixTextBaseLine(String suffixText) {
        ...
        switch (mSuffixGravity) {
            ...
            case 3:
                timeLineBottom = true;
                // 时间的正下方
                ret = mTimeTextBaseline + tempRect.bottom + mSuffixTopMargin;
                textBoundWidth = tempRect.width();
                break;
        }
    ...
    private float getDayAndHourContentWidth() {
        ...
        if (isShowDay) {
            String tempDay = CountUtils.formatNum(mDay);
        ...
        if (isShowHour) {
            String tempHour = CountUtils.formatNum(mHour);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(CountUtils.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixDayTextWidth > 0) {
                // draw day suffix
                if (timeLineBottom){
                    float curX = mLeftPaddingSize + mDayTimeTextWidth / 2-textBoundWidth/2;
                    canvas.drawText(mSuffixDay, curX, mSuffixDayTextBaseline, mSuffixTextPaint);
                }else {
            ...
            // hour left point
            if (timeLineBottom){
                mHourLeft = mLeftPaddingSize + mDayTimeTextWidth +suffixMargin;
            }else {
        ...
        if (isShowHour) {
            ...
            float curX = mHourLeft +(mTimeTextWidth - textBoundWidth)/2;
            canvas.drawText(CountUtils.formatNum(mHour), mHourLeft + curTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixHourTextWidth > 0) {
                // draw hour suffix
                if (timeLineBottom){
                    canvas.drawText(mSuffixHour, curX, mSuffixHourTextBaseline, mSuffixTextPaint);
                }else {
            ...
            if (timeLineBottom ){
                // minute left point
                mMinuteLeft = mHourLeft + mSuffixHourTextWidth +suffixMargin;
                if (!isShowDay){
                    mMinuteLeft =mMinuteLeft+curTimeTextWidth/3 - textBoundWidth/2;
                }
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(CountUtils.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2 , mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixMinuteTextWidth > 0) {
                // draw minute suffix
                if (timeLineBottom){
                    float curX = mMinuteLeft+textBoundWidth/4;
                    canvas.drawText(mSuffixMinute, curX, mSuffixMinuteTextBaseline, mSuffixTextPaint);
                }else {
            ...
            if (timeLineBottom){
                mSecondLeft = mMinuteLeft + mTimeTextWidth +suffixMargin;
                if (!isShowDay){
                    mSecondLeft =mSecondLeft+mSuffixMinuteTextWidth/3 - textBoundWidth/2;
                }
            }else {
        ...
        if (isShowSecond) {
            // draw second text
            canvas.drawText(CountUtils.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
                if (timeLineBottom){
                    float curX = mSecondLeft +textBoundWidth/4;
                    canvas.drawText(mSuffixSecond,curX, mSuffixSecondTextBaseline, mSuffixTextPaint);
                }else {
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(CountUtils.formatMillisecond(mMillisecond), mMillisecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = CountUtils.sp2px(mContext, textSize);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = CountUtils.sp2px(mContext, textSize);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = CountUtils.dp2px(mContext, suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = CountUtils.dp2px(mContext, suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = CountUtils.dp2px(mContext, suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = CountUtils.dp2px(mContext, suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = CountUtils.dp2px(mContext, suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = CountUtils.dp2px(mContext, suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = CountUtils.dp2px(mContext, suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = CountUtils.dp2px(mContext, suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = CountUtils.dp2px(mContext, suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = CountUtils.dp2px(mContext, suffixMillisecondMarginL);
            isRef = true;
        }
