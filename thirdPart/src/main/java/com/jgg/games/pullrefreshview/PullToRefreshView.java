...
public class PullToRefreshView extends ViewGroup{
    ...
    private boolean canLoadMore = true; // 是否可以加载
    ...
    protected void onFinishInflate() {
        ...
        if (mHeaderIndicator == null) {
            mHeaderIndicator = new DefaultHeader(context);
        }
        ...
        if (mFooterIndicator == null) {
            mFooterIndicator = new DefaultFooter(context);
        }
...
//            }
    ...
    public boolean canMove(){
        if ((mStatus == PULL_DOWN || mStatus == IDLE) && canRefresh){
            return true;
        }else if ((mStatus == PULL_UP || mStatus == IDLE) && canLoadMore){
            return true;
        }
        return false;
    }
